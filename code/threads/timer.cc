...
Timer::CallBack() 
{
	...
	Thread * tempThread;
     IntStatus old=kernel->interrupt->getLevel();
       kernel->interrupt->SetLevel(IntOff);

    tempThread=kernel->scheduler->FindNextToRun();
    kernel->interrupt->SetLevel(old);

    if(tempThread->priority<kernel->currentThread->priority){
	old=kernel->interrupt->getLevel();
       kernel->interrupt->SetLevel(IntOff);
       

       kernel->scheduler->ReadyToRun(tempThread);
